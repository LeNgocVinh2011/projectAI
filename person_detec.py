...
def predict_folder(video_path, second):
    # Read video file
    ...
    for frame_idx in range(int(cap.get(cv2.CAP_PROP_FRAME_COUNT))):
        if second != None:
            cap.set(cv2.CAP_PROP_POS_MSEC, (counts * int(second)))
        ret, frame = cap.read()
        if frame is None:
            break
        image_array = frame
        ...
        T2 = time.time()
        FPS = 1.0/(T2-T1)
        print('+ Frame: ' + str(counts) + ', FPS: ' + str(FPS) + ', boxes: ', len(boxes))
        counts = counts + 1
        fps_sum += FPS
        ...
        if len(boxes) != 0:
            ...
            cv2.imwrite(pathFile + '/' + file_name[0] + f"{counts}.jpg", frame_copy)
        show_img = 1
        if show_img == 1:
            image_data = np.asarray(img)
            cv2.imshow("Video Detection", cv2.cvtColor(image_data, cv2.COLOR_RGB2BGR))
        key = cv2.waitKey(1)
        if key == ord('q') or frame_idx == range(int(cap.get(cv2.CAP_PROP_FRAME_COUNT))):
            break
    print('+ mean FPS: ', fps_sum/counts)
    cv2.destroyAllWindows()
    filedialog.askopenfilename(initialdir=os.path.normpath(pathFile), title="Open frame saved")
